...
private:
  ...
  static int size_impl(const Node *node) {
    ...
    return 1 + size_impl(left) + size_impl(right);
  }
  ...
  static int height_impl(const Node *node) {
    ...
    Node *left = node->left;
    Node *right = node->right;
    return 1 + max(height_impl(left), height_impl(right));
  }
  ...
  static void destroy_nodes_impl(Node *node) {
    ...
    destroy_nodes_impl(left);
    destroy_nodes_impl(right);
  }
  ...
  static Node * min_element_impl(Node *node) {
    if (empty_impl(node) || empty_impl(node->left)) {
      return node;
    }
  ...
  static Node * max_element_impl(Node *node) {
    if (empty_impl(node) || empty_impl(node -> right)) {
      return node;
    }
