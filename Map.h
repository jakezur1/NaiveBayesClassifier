  ...
  class PairComp {
    ...
    public:
      bool operator()(const Pair_type& left,const Pair_type& right) const {
        return check(left.first, right.first);
      }
  };
...
public:
  ...
  Iterator find(const Key_type& k) const {
    Pair_type checking(k,Value_type());
    Iterator iter = tree.find(checking);
    if (iter != tree.end()) {
      return iter;
    } else {
      return tree.end();
  ...
  Value_type& operator[](const Key_type& k) {
    ...
    if (iter != tree.end()) {
      ...
      // removed tree.insert and using map insert here instead
      insert({k, Value_type()});
      iter = find(k);
      return iter->second;
    }
